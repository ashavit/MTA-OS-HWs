...
#define TRY_TIME (5 * (1 + getpid() % 2))
#define STORE_GET_TIME (3 * (1 + getpid() % 3))
#define STORE_RETURN_TIME (1 + getpid() % 3)
#define PAY_TIME 3
...
int main(int argc, char* argv[])
{
	...
	while ((ret = getline(&buf, &char_count, fp)) != -1) {
		printf("Shopper: read line: %s", buf);
		proc_count++;
		pid=fork();
		if (pid<0)
		{
	...
	while(proc_count > 0)
	{
		...
		wait(&status);
		proc_count--;
		//if (WIFEXITED(status)) proc_count--;
	}
